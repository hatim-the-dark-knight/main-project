...
def get_trainer(model, optimizer, cfg, device, generator=None):
    ...
    '''
    set_logger(cfg)
    trainer = dvr.config.get_trainer(
...
def get_generator(model, cfg, device):
    ...
    Args:
        ...
        device (device): pytorch device
    '''
    generator = dvr.config.get_generator(model, cfg, device)
    return generator
...
def get_renderer(model, cfg, device):
    ...
    Args:
        ...
        device (device): pytorch device
    '''
    renderer = dvr.config.get_renderer(model, cfg, device)
    return renderer
